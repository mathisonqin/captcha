...
var (
	...
	mapDifficultIdentifyChar = map[byte]bool{
		0:  true,
		1:  true,
		13: true,
		14: true, //E
	...
	mapDifficultToEasy = map[byte]byte{
		0:  2,
		1:  3,
		13: 11, //D -> B
		14: 22, //E -> M
...
func NewLen(length int) (id string) {
	...
	value = filterDifficultIdentifyChar(value) //filter the char which is difficult to identify
	//value = changeNumToChar(value)             //change 10-35 to A-Z
	globalStore.Set(id, value)
	return
}

//filter the char which is difficult to identify,and change 10-35 to A-Z
func filterDifficultIdentifyChar(value []byte) []byte {
	lenValue := len(value)
	for i := 0; i < lenValue; i++ {
		if mapDifficultIdentifyChar[value[i]] == true {
			value[i] = mapDifficultToEasy[value[i]]
		}
		if value[i] >= 10 {
			value[i] = value[i] - 10 + byte('A')
		}
...
func WriteImage(w io.Writer, id string, width, height int, fixColor bool) error {
	...
	_, err := NewImage(id, d, width, height, true, true, false, fixColor).WriteTo(w)
	return err
}
